...
def load_tokenizer(encoder_name: str = 'gpt-4-turbo') -> tiktoken.Encoding:
    """
        ...
        Args:
        encoder_name (str): The name of the encoder model to be used for tokenization. Default is 'gpt-4'.
...
def split_pages_into_chunks(pages:List[str], chunk_size:int, tokenizer:tiktoken.Encoding) -> List[str]:
    ...
    pages_tokens:List[List[int]] = [tiktoken.encode(page) for page in pages]
    total_tokens = list(it.chain(*pages_tokens))
    
    nb_tokens = len(total_tokens)
    nb_chunks = round(nb_tokens/chunk_size)
    
    accumulator:List[str] = []
    
    for chunk_tokens in np.array_split(total_tokens, nb_chunks):
...
def find_embedding_candidates(query_embedding:np.ndarray, chunks:List[str], corpus_embeddings:np.ndarray, top_k:int=7)->List[str]:
    ...
    # Compute the cosine similarity scores by dividing the dot products by the product of the norms.
...
def chatgpt_completion(context:str, query:str):
    """
        ...
        Returns:
        - completion_rsp: The response from the GPT-4 Turbo model. This is an object containing the model's reply to the input query.
    """
